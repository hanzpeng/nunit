...
namespace NUnitTests {
    public class Solution1 {
        public int LongestIncreasingPath(int[][] matrix) {
            ...
            for (int r = 0; r < R; r++) {
                for (var c = 0; c < C; c++) {
                    maxDepth = Math.Max(maxDepth, Dfs(r, c, matrix, depth));
                }
            }
            return maxDepth;
        }

        int Dfs(int r, int c, int[][] matrix, int[][] depth) {
            ...
            foreach (var d in dir) {
                var r1 = r + d.dr;
                var c1 = c + d.dc;
                if (r1 >= 0 && r1 < R && c1 >= 0 && c1 < C && matrix[r1][c1] > matrix[r][c]) {
                    maxDepth = Math.Max(maxDepth, Dfs(r1, c1, matrix, depth) + 1);
                }
            }
            depth[r][c] = maxDepth;
            return maxDepth;
